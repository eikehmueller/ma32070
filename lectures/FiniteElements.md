...
For $=2$ there are six basis functions which are visualised in the following figure (red arrows indicate gradients):

![Basis functions for quadratic finite element](figures/quadratic_element.png)


## Formal definition of finite elements
It turns out that it is advantageous to define finite elements in a more general sense. For this we first need to introduce the notion of the dual for a function space.
### Dual spaces
Consider a domain $K$ and the space $P$ of real-valued functions $w:K\rightarrow \mathbb{R}$ on $K$. A linear *functional* $\ell$ maps a function $w\in P$ to a real value such that

$$
\ell(c_1 w^{(1)}+c_2 w^{(2)}) = c_1\ell(w^{(1)})+c_2 \ell(w^{(2)}) \qquad\text{for all $c_1,c_2\in\mathbb{R}$, $w^{(1)}, w^{(2)} \in P$}
$$

The space of all linear functionals on $P$ is called the **dual space** $P^*$.

#### Examples
Let $K\subset \mathbb{R}^2$ and $P=H^1(K)$. Then the following $\ell$ are linear functionals:
* $\ell(w) := w(\xi)$ for some point $\xi\in K$
* $\ell(w) := \frac{\partial w}{\partial x_0}$
* $\ell(w) := \int_K f(x)w(x)$ for some function $f(x)\in L_2(K)$

### Ciarlet's definition of the finite element
A finite element is a triple $(K,\mathcal{V},\mathcal{L})$ which consists of
* the **domain** $K$
...
(the definition follows [Logg2012])
...
* $\mathcal{V} = P_5$, the space of quintic bi-variate polynomials
* 21 nodes defined as follows:
  - $\ell_i(w) = w(v_j)$ (evaluation at each vertex $v_i$ $\Rightarrow$ 3 nodes)
  - $\ell_{3+2i+j}(w) = \frac{\partial w}{\partial x_j}(v_i)$ (gradient evaluation at each vertex $\Rightarrow$ 6 nodes)
...
## Vandermonde matrix
How can we construct the basis functions $\phi_k(x)$ for a given set of nodes $\{\ell_j\}_{j=0}^{d-1}$? For this, assume that we know some set of basis functions $\{b_i(x)\}_{i=0}^{d-1}$ of $\mathcal{V}$. For the Lagrange elements, these could for example be the monomials $1,x_0,x_1,x_0^2,x_0x_1,x_1^2,\dots$. Hence, we can write

$$
\phi_k(x) = \sum_{i=0}^{d-1} c_i^{(k)} b_i(x)
$$

for some coefficients $c_i^{(k)}$. Since $\{\phi_k\}_{k=0}^{d-1}$ is a nodal basis and $\ell_j$ are linear functionals we know that
