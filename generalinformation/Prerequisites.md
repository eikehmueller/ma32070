...
```Python
T = np.zeros(shape=(2,3,4))
```

or construct the $2\times 3$ matrix $\begin{pmatrix}1 & 2 & 3 \\ 4 & 5 & 6\end{pmatrix}$ like this:
...
Tensors can be multiplied with the [`numpy.einsum` function](https://numpy.org/doc/stable/reference/generated/numpy.einsum.html). For example, to compute $R_{ijm\ell n} = \sum_{k} T_{ijk} T'_{mk\ell n}$ use
...
`numpy.einsum` is very powerful, look at [the documentation](https://numpy.org/doc/stable/reference/generated/numpy.einsum.html) for further details.
