...
\begin{center}
  Implement Finite Element Method (FEM) in 2 dimensions
\end{center}
\end{frame}
...
\item mixture of implementation tasks, mathematical derivations and numerical experiments
\item due in week 11
\end{itemize}
\vspace{2ex}
...
\begin{frame}[fragile]
\frametitle{Noteable setup}
Locally installed packages are \textbf{not persistent} on Noteable.\\
$\Rightarrow$ need to run
{\color{blue}
\begin{verbatim}
  python -m pip install --editable .
\end{verbatim}}
in \verb!ma32070/finiteelements! every time you shut down and restart Noteable.\\[2ex]
Run 
{\color{blue}
\begin{verbatim}
  git pull
\end{verbatim}}
at regular intervals to pull changes to the finite element library.
\end{frame}
...
\textbf{Mathematical/programming background} {\footnotesize($\rightarrow$ moodle for details)}
\begin{itemize}
...
$$
  ...
  \begin{itemize}
    \item Correctness
    \item Efficiency
    \item Maintainability\\$\qquad\Rightarrow$ suitable abstractions
  \end{itemize}
...
\item Bugs!
\item Rounding errors (numerical instability)
\end{itemize}
\end{frame}
...
$$
\begin{itemize}
  \item Values: $V=[\underline{\textcolor{red}{1.3}, 2.4, 8.7}, \underline{\textcolor{red}{4.5}, 6.1}, \underline{\textcolor{red}{2.1}, 8.3, 9.4}, \underline{\textcolor{red}{3.7}, 1.1, 7.7}]$
  \item Column indices: $J=[\underline{\textcolor{red}{0},1,3},\underline{\textcolor{red}{0},1},\underline{\textcolor{red}{1},2,4},\underline{\textcolor{red}{1},2,4}]$
...
$S = [n]$ \quad ($\rightarrow$ dimension)
\item[rank 2 = matrices] $A\in \mathbb{R}^{n\times m}$, $A_{ij}$ for $0\le i<n$, $0\le j <m$\\ $S=[n,m]$ \quad ($\rightarrow$ \# rows, \#columns)
\item[rank 3 = set of matrices] $\{A^{(\ell)}\}_{\ell=0}^{k-1}$, $A^{(\ell)}\in \mathbb{R}^{n\times m}$, $A_{\ell ij}:=A^{(\ell)}_{ij}$\\ $S=[k,n,m]$ \quad ($\rightarrow$ \# matrices,\# rows, \#columns)
\end{description}
\end{frame}
...
T = np.zeros(shape=[2,3,4],dtype=float)    
  \end{lstlisting}
  \item $2\times 3$ matrix $\begin{pmatrix}1.8 & 2.2 & 3.4 \\ 4.2 & 5.1 & 6.7\end{pmatrix}$ $\Rightarrow$ array of shape $[2,3]$
\begin{lstlisting}
...
\end{lstlisting}
\end{description}
{\begin{flushright} {\footnotesize [end of week 1]} \end{flushright}}
\end{frame}
...
\textbf{Elliptic PDE} Find function $u$ such that
$$
...
\begin{minipage}{0.25\linewidth}
  \begin{center}
  \includegraphics[width=\linewidth]{figures/domain.pdf}
  \end{center}
\end{minipage}
\begin{itemize}
...
Symmetric bilinear form\footnote{
  {\tiny
  $a(c_1 u^{(1)} + c_2 u^{(2)},v) = c_1 a(u^{(1)},v) + c_2 a(u^{(2)},v)$, $a(u,c_1 v^{(1)} + c_2 v^{(2)}) = c_1 a(u,v^{(1)}) + c_2 a(u,v^{(2)})$\\for all $c_1,c_2\in \mathbb{R}$, $u,v,u^{(1)},u^{(2)},v^{(1)}, v^{(2)} \in \mathcal{V}$ and $a(v,u) = a(u,v)$ for all $u,v\in \mathcal{V}$}} $a(\cdot,\cdot): \mathcal{V}\times \mathcal{V} \rightarrow \mathbb{R}$
$$
...
\item $\min_{v_h\in \mathcal{V}_h}\|u-v_h\|_{\mathcal{V}}$ depends on $\mathcal{V}_h$
\end{itemize}
...
T^\partial_{r\ell a}(\boldsymbol{\zeta}) &:= \frac{\partial \phi_\ell}{\partial x_a}(\zeta^{(r)}) 
 ...
 \end{aligned}
$$
{\begin{flushright} {\footnotesize [end of week 2]} \end{flushright}}
\end{frame}
...
What do we need?
\begin{itemize}
\item Evaluate degrees of freedom (nodes) for arbitrary functions $f$
$$
\lambda_\ell(f)\in \mathbb{R}
$$
\item Tabulate basis functions $\phi_\ell$ and derivatives at points $\zeta^{(r)}$
$$
T_{r\ell}(\boldsymbol{\zeta}) = \phi_\ell(\zeta^{(r)})\quad {\footnotesize [n\times \nu]},\qquad
T^\partial_{r\ell a}(\boldsymbol{\zeta}) = \frac{\partial \phi_\ell}{\partial x_a}(\zeta^{(r)})\quad {\footnotesize [n\times \nu\times 2]}
$$
\item \# unknowns associated with vertices $\nu_{\text{vertex}}$, facets $\nu_{\text{facet}}$ and interior $\nu_{\text{interior}}$\\
$\Rightarrow$ Indirection map $\mu_{\text{dof}}$: $\lambda_{\ell=\mu_{\text{dof}}(E,\rho,j)} = \lambda_j^{(E_\rho)}$
\end{itemize}
\end{frame}
...
\begin{itemize}
\item \# dofs associated with topological entities:\\\lstinline{ndof_per_vertex} = $\nu_{\text{vertex}}$, \lstinline{ndof_per_facet} = $\nu_{\text{facet}}$ and \lstinline{ndof_per_interior} = $\nu_{\text{interior}}$
\item Evalute all dofs for given function $\widehat{f}$: \lstinline{tabulate_dofs(fhat)} 
\item Tabulate basis functions: $T$ \lstinline{tabulate(zeta)}
\item Tabulate gradients of basis functions: $T^\partial$ \lstinline{tabulate_gradient(zeta)}
\end{itemize}
\end{frame}
...
\item Cubic element $(p=3)$: \lstinline{CubicElement} (= exercise)
...
Weak form of the PDE requires \textbf{integration} over domain $\Omega$
$$
a(u,v) = \int_\Omega \left(\kappa \nabla u \cdot \nabla  v + \omega\; u v\right)\;dx
$$
...
$$
\textbf{Quadrature\footnote{often called ``cubature'' in more than one dimension} rule} $\mathcal{Q}=\{(\widetilde{\zeta}^{(q)},\widetilde{w}_q)\}_{q=0}^{n_q-1}$
\begin{itemize}
...
\begin{center}
  ...
  \begin{tabular}{cccc}
    ...
    \hline\hline
 1   & $0$ & $2$ & $1$\\
 2   & $-\sqrt{\frac{1}{3}}, +\sqrt{\frac{1}{3}}$ & $1,1$ & $3$\\
...
\end{itemize}
{\begin{flushright} {\footnotesize [end of week 3]} \end{flushright}}
\end{frame}
...
$$
\begin{aligned}
-\nabla\cdot(\kappa \cdot \nabla u) + \omega u &= f\qquad\text{in $\Omega=\widehat{K}$}\\
...
$$
x = \pm \left(d_0 + d_1\beta^{-1} + d_2\beta^{-2} + \dots+d_{p-1}\beta^{1-p}\right)\cdot\beta^E
$$
...
\end{aligned}
$$
Observe: $z'\ll x'\sim y'$ in contrast to $z\sim x\sim y$ (Example 1)
\end{frame}
...
$$
$$
\Rightarrow \quad x_1 = 1 \quad \Rightarrow\quad  x_1 = 1 +10^{-20} x_0 = 1 \quad \Rightarrow \quad x_0 = 0
$$
...
\end{pmatrix}}
$$
Exact solution, independent of $\epsilon$
$$
...
{\begin{flushright} {\footnotesize [end of week 4]} \end{flushright}}
\end{frame}
...
\textbf{Theorem 1}\quad{\footnotesize [J Demmel: \textit{``Numerical Linear Algebra''}, SIAM 1997]}\\
...
$$
Properties of $\|\cdot\|_\infty$ applied to $\vec{b} = A\vec{u}$, $\vec{r} = A\;\delta \vec{u}$
\begin{xalignat*}{2}
...
\begin{frame}[fragile]
  ...
  \end{itemize}
  {\begin{flushright} {\footnotesize [end of week 5]} \end{flushright}}
\end{frame}
...
Derivatives $\Rightarrow$ $n \times \nu^\times \times 2\times 2$ rank-4 tensor $T^{\times\partial}$ 

$$
{\footnotesize
T^{\times\partial}_{r\ell^\times ab}(\boldsymbol{\zeta}) := \frac{(\partial \phi^\times_{\ell^\times})_a}{\partial x_b}(\zeta^{(r)})  = \begin{cases}
\frac{\partial\phi_{\frac{\ell^\times}{2}}}{\partial x_b}(\zeta^{(r)}) = T_{r,\frac{\ell^\times}{2},b}& \text{if $\ell^\times$ even, $a=0$} \\[2ex]
...
$b^{(h),\text{local}}_{\ell} = \sum_q w_q T_{q\ell}F_q  D_q$}
...
\begin{frame}[fragile]
  ...
  Summation $\sum_q w_q T_{q\ell} F_q D_q$ $\Rightarrow$ \lstinline{einsum()}\\
  \begin{lstlisting}
...
&\hspace{7cm}\cdot|\det{J}(\widehat{x})|d\widehat{x}
\\
&\approx \sum_{K\in \Omega_h}\sum_q  w_q \Big(\kappa \widehat{\nabla} \phi_k(\xi^{(q)})(J^{\top}(\xi^{(q)}) J(\xi^{(q)}))^{-1}\widehat{\nabla}\phi_\ell(\xi^{(q)})\\[-1.5ex]
&\qquad\qquad\qquad+\;\; \omega\phi_k(\xi^{(q)})\phi_\ell(\xi^{(q)})\Big)|\det{J}(\xi^{(q)})| \\
...
\begin{frame}[fragile]
  ...
  \item $\sum_{qab}w_q  T^\partial_{qka}(J^{(-2)}_q)_{ab} T^\partial_{q\ell b} D_q$
  \item $\sum_{q}w_q  T_{qk} T_{q\ell} D_q$
  \end{itemize}
...
with measure_time("assemble stiffness matrix"):
    A_h = assemble_lhs(fs, quad, kappa, omega)
\end{lstlisting}
{\begin{flushright} {\footnotesize [end of week 6]} \end{flushright}}
\end{frame}
...
\item scale first row by $\rho_i$, add scaled first row to row $i$\\
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Row-pointers $R$ $\Rightarrow$ $n+1$ numbers
  \end{itemize}
...
{\begin{flushright} {\footnotesize [end of week 7]} \end{flushright}}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    $$
  \end{itemize} 
  \textbf{Converging} method\quad {\footnotesize (direct solver = special case $\boldsymbol{u}^{(1)}=\boldsymbol{u}$)}
  $$
...
\end{itemize}
Construction of efficient preconditioners is
\begin{itemize}
\item an art
...
\begin{frame}
  ...
  Jacobi preconditioner $P=D$ (diagonal of $A$)
$$
D_{ij} = \begin{cases}
A_{ii} & \text{if $i=j$}\\
0 & \text{otherwise}
...
\end{verbatim}}
{\begin{flushright} {\footnotesize [end of week 8]} \end{flushright}}
\end{frame}
...
&\approx \frac{2}{3}n^3 t_{\text{flop}}\left(1 + \frac{3}{2n}\frac{t_{\text{mem}}}{t_{\text{flop}}} \right)
...
\begin{frame}
  ...
  \begin{itemize}
    \item Converges in single iteration
...
\end{itemize} 
{\begin{flushright} {\footnotesize [end of week 9]} \end{flushright}}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Simulate evolution of state of the atmosphere
...
\begin{frame}
  ...
  \item $10^6\times-10^7\times$ faster than our computer $R=6.3\cdot 10^{10}$ FLOPs
  \end{itemize}
...
\item $n_{\text{local}} \times m_{\text{local}}$ + $m_{\text{local}}\times r$ memory reads (matrices $A_{pq}$, $B_q$)
...
$$
\begin{center}
Computation time (mostly) reduced by factor $P$\\{\footnotesize(compared to sequential case)}
\end{center}
\end{frame}
...
$$
\begin{aligned}
T(P) &= T_{\text{compute}} + T_{\text{comm}}\\
&= \underbrace{\frac{2n^3 t_{\text{flop}}+n^2 t_{\text{mem}}}{P}}_{A} + \underbrace{2n^2 t_{\text{mem}} + n^2 t_{\text{word}} + P t_{\text{lat}}}_{B}
\end{aligned}
$$
